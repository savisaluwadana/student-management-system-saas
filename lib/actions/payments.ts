...
async function sendPaymentConfirmationMessage(
  ...
  try {
    ...
    const whatsappTo = normalizePhoneNumber(studentData.whatsapp_phone);
    const smsTo = normalizePhoneNumber(studentData.phone) || normalizePhoneNumber(studentData.guardian_phone) || whatsappTo;
    ...
    const message = `Payment received: ${formatPaymentAmount(Number(payment.amount))} from ${studentData.full_name}${methodText} on ${dateText}. Thank you.`;
...
async function validatePaymentRelations(input: Partial<CreatePaymentInput>, context: WorkspaceContext) {
  if (input.student_id) {
    if (!mongoose.isValidObjectId(input.student_id)) return 'Invalid student.';
    const student = await Student.exists(workspaceFilter(context, { _id: input.student_id }));
    if (!student) return 'Student does not belong to this workspace.';
  }

  if (input.class_id) {
    if (!mongoose.isValidObjectId(input.class_id)) return 'Invalid class.';
    const classItem = await Class.exists(workspaceFilter(context, { _id: input.class_id }));
    if (!classItem) return 'Class does not belong to this workspace.';
  }
...
export async function getPayments(status?: string): Promise<FeePaymentType[]> {
  await connectDB();
  ...
  const payments = await FeePayment.find(workspaceFilter(context, filter))
    .sort({ created_at: -1 })
  ...
  return (payments as any[]).map((payment) => ({
    ...
    students: {
      id: payment.student_id?._id?.toString(),
      full_name: payment.student_id?.full_name,
      student_code: payment.student_id?.student_code,
    },
  })) as unknown as FeePaymentType[];
}
...
export async function createPayment(input: CreatePaymentInput): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    const relationError = await validatePaymentRelations(input, context);
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const [totalStudents, activeClasses, paidPayments, pendingPayments] = await Promise.all([
    ...
    FeePayment.find(workspaceFilter(context, { status: 'paid' })).select('amount').lean(),
    FeePayment.find(workspaceFilter(context, { status: { $in: ['pending', 'unpaid', 'overdue'] } })).select('amount').lean(),
  ]);

  const totalRevenue = (paidPayments as any[]).reduce((sum, payment) => sum + (payment.amount || 0), 0);
  const pendingAmount = (pendingPayments as any[]).reduce((sum, payment) => sum + (payment.amount || 0), 0);
...
export async function generateMonthlyFees(targetMonth: Date): Promise<{ success: boolean; error?: string; count?: number }> {
  ...
  try {
    const enrollments = await Enrollment.find(workspaceFilter(context, { status: 'active' }))
    ...
    for (const enrollment of enrollments as any[]) {
      ...
      const existing = await FeePayment.findOne(workspaceFilter(context, {
